...
def _index_from_tickers(ticker_dir: Path, sp500_csv: Optional[Path]) -> Tuple[List[Dict[str, Any]], Dict[str, float]]:
    # Optionally ingest name/sector/mcap from a CSV universe (not required)
    ...
    if sp500_csv and sp500_csv.exists():
        try:
            ...
            for _, r in df.iterrows():
                ...
                if namecol:
                    name_map[s] = str(r[namecol])
                if sectcol:
                    sect_map[s] = str(r[sectcol])
                if mcapcol:
                    try:
                        mcap_map[s] = float(r[mcapcol])
                    except Exception:
                        pass
        except Exception:
            pass
...
def _fetch_missing_mcaps(symbols: List[str], have: Dict[str, float]) -> Dict[str, float]:
    ...
    for s in missing:
        try:
            ...
            # tolerate 404s (e.g., WBA), delistings, etc.
            pass
    return have
    ...
    for f in sorted(ticker_dir.glob("*.json")):
        ...
        for row in daily:
            ...
            if row.get("pred") is not None:
                try:
                    pm[str(d)] = float(row["pred"])
                except Exception:
                    pass
            if row.get("score") is not None:
                try:
                    sm[str(d)] = float(row["score"])
                except Exception:
                    pass
        if pm:
            preds[sym] = pm
...
def _build_long_only(preds: Dict[str, Dict[str, float]],
                     scores: Dict[str, Dict[str, float]],
                     rets: Dict[str, Dict[str, float]],
                     top_n: int = 50) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    all_dates = set()
...
def main():
    ...
    # ----- write index.json (keep existing schema + add sp500) -----
    ...
    # Save the raw benchmark prices too (optional for frontend)
    ...
    # ----- comparison series (portfolio equity vs SP500 equity) -----
    comp = []
    ...
    # ----- write portfolio.json with legacy top-level keys -----
    portfolio_payload = {
        # legacy fields (your UI currently reads these):
        "points": len(curve),
        "equity_curve": curve,
        "daily": daily,
        "long_n": args.top_n,
        ...
        # richer fields (new, backward-compatible):
        "metrics": p_metrics,
