...
from market_sentiment.news import _prov_finnhub, _prov_yfinance, fetch_news
...
def main():
    ...
    for t in tickers:
        # pull both providers and merge; OR call fetch_news which does exactly that
        try:
        ...
        if len(n_fh) or len(n_yf):
            ...
            # additional fallback: use fetch_news (same providers) in case one wrapper path fails in some envs
            try:
                n = fetch_news(t, a.start, a.end, None, max_per_provider=a.news_per_provider)
            except Exception:
