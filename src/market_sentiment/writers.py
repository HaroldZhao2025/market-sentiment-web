...
def build_ticker_json(ticker: str, price_df: pd.DataFrame, daily_sent: pd.DataFrame, recent_news: pd.DataFrame) -> dict:
    ...
    series = (
        p[["date","close"]]
        .merge(d[["date","S","news_count"]] if "news_count" in d.columns else d[["date","S"]], on="date", how="left")
        .sort_values("date")
        .reset_index(drop=True)
    )
    series["S"] = series["S"].fillna(0.0)
    series["news_count"] = series.get("news_count", pd.Series([0]*len(series))).fillna(0).astype(int)

    # 7-day rolling sentiment and news counts
    ...
    # A slightly stronger mapping → more visible KPI
    predicted_return = float(np.tanh(last_S7 / 3.0) * 0.015)  # ~±1.5% cap
    ...
    top_news = (news.head(30)[["ts","title","source","url"]] if not news.empty
    ...
    return {
        ...
        "insights": {
            ...
            "advisory": (
                "Strong Buy" if last_S7 > 2 else
                ("Buy" if last_S7 > 0.5 else ("Hold" if last_S7 > -0.5 else "Sell"))
            ),
        },
        "series": {
