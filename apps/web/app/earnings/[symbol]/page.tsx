...
async function loadCompanies(): Promise<CompanyMeta[]> {
  ...
  const data = await readJson<{ companies?: CompanyMeta[] }>(path.join(publicData, "v5", "universe.json"));
...
function legacyToCurrent(symbol: string, raw: any): EarningsArtifact {
  ...
  return { schema_version: 1, symbol, earnings_history: [], calls: [], call_links: [], filing_fallback: docs.map((doc: any) => ({ ts: String(doc?.ts ?? ""), title: String(doc?.title ?? ""), url: String(doc?.url ?? ""), source: String(doc?.source ?? "Legacy earnings source"), document_type: "legacy", S: typeof doc?.S === "number" ? doc.S : undefined })) };
}
...
export async function generateStaticParams() { return (await loadSymbols()).map((symbol) => ({ symbol })); }
...
export default async function EarningsPage({ params }: { params: { symbol: string } }) {
  ...
  return <main className="space-y-7">
    <section className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><span className={`pill ${callCount > 0 ? "text-emerald-300" : ""}`}>{callCount} structured call{callCount === 1 ? "" : "s"}</span>{sourceCount > 0 ? <span className="pill">{sourceCount} public source{sourceCount === 1 ? "" : "s"}</span> : null}<Link href={`/ticker/${symbol}#earnings`} className="pill">Open company workspace →</Link></div>
    </section>
