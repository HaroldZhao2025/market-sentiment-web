...
export default function EarningsIntelligenceClient({ symbol, data }: { symbol: string; data: EarningsArtifact }) {
  ...
  const callLinks = Array.isArray(data.call_links) ? data.call_links.filter((item) => item?.url && item?.title) : [];
    ...
    <div className="space-y-7">
      ...
      {callLinks.length ? (
        <section className="space-y-3">
          <div><div className="eyebrow">Public call sources</div><h2 className="section-title mt-1">Transcript and call links</h2><p className="section-copy">These links are discovery evidence. A link is not counted as a structured call until usable transcript text has been parsed and scored.</p></div>
          <div className="grid gap-3 md:grid-cols-2">
            {callLinks.slice(0, 10).map((item, index) => <a key={`${item.url}-${index}`} href={item.url} target="_blank" rel="noreferrer" className="card card-hover p-4"><div className="text-[11px] text-neutral-600">{dateOnly(item.ts)} · {item.source || item.provider || "Public source"}</div><div className="mt-2 text-sm font-medium leading-6 text-neutral-200">{item.title || "Earnings call source"}</div></a>)}
          </div>
        </section>
      ) : null}
