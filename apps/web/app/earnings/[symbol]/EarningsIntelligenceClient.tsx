...
type Call = {
  ...
  summary?: { turn_count?: number; scored_turn_count?: number; overall_sentiment?: number | null; prepared_sentiment?: number | null; qa_sentiment?: number | null; qa_tone_shift?: number | null; uncertainty_turn_rate?: number | null; forward_looking_turn_rate?: number | null; topics?: Topic[] };
...
export type EarningsArtifact = { schema_version?: number; symbol?: string; earnings_history?: EarningsHistory[]; calls?: Call[]; call_links?: CallLink[]; filing_fallback?: Filing[]; methodology?: Record<string, unknown> };
...
function signed(value: number | null, digits = 3) { return value == null ? "—" : `${value > 0 ? "+" : ""}${value.toFixed(digits)}`; }
...
function linkKind(item: CallLink) {
  ...
  return title.includes("transcript") ? "Public transcript" : title.includes("webcast") || title.includes("replay") ? "Webcast / replay" : "Call source";
}

export default function EarningsIntelligenceClient({ symbol: _symbol, data }: { symbol: string; data: EarningsArtifact }) {
  ...
  const usableCalls = complete.length ? complete : calls;
  const [callIndex, setCallIndex] = useState(0);
  const [section, setSection] = useState<"all" | "prepared" | "qa">("all");
  const [turnLimit, setTurnLimit] = useState(20);
  const call = usableCalls[Math.min(callIndex, Math.max(0, usableCalls.length - 1))] ?? null;
  const summary = call?.summary ?? {};
  const isComplete = call ? completeCall(call) : false;
  const turns = useMemo(() => {
  ...
  return <div className="space-y-7">
    {isComplete ? <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
    ...
    </section> : <section className="rounded-2xl border border-white/10 bg-white/[0.025] p-5 md:p-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div><div className="eyebrow">Call status</div><h2 className="mt-2 text-xl font-semibold text-white">{calls.length ? "Structured call is incomplete" : links.length ? "Public call sources found" : "No structured free-public transcript yet"}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-neutral-500">Prepared/Q&amp;A analytics appear only when free-public transcript text is successfully parsed. Filing and news links are never presented as transcript text.</p></div>
    ...
    {links.length ? <section className="space-y-3">
      <div><div className="eyebrow">Public sources</div><h2 className="section-title mt-1">Call and transcript links</h2><p className="section-copy">A public link is shown as a source; it counts as a structured call only after transcript parsing succeeds.</p></div>
      <div className="grid gap-3 md:grid-cols-2">{links.slice(0, 12).map((item, index) => <a key={`${item.url}-${index}`} href={item.url} target="_blank" rel="noreferrer" className="card card-hover block p-4"><div className="flex items-center justify-between gap-3"><span className="rounded-lg border border-white/10 px-2 py-1 text-[10px] uppercase tracking-wider text-neutral-500">{linkKind(item)}</span><span className="text-[11px] text-neutral-600">{dateOnly(item.ts)}</span></div><div className="mt-3 text-sm font-medium leading-6 text-neutral-200">{item.title}</div><div className="mt-2 text-[11px] text-neutral-600">{item.source || item.provider || "Public source"}</div></a>)}</div>
    </section> : null}

    {call ? <section className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
      <div className="card p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="eyebrow">Call diagnostics</div><h2 className="section-title mt-1">Management tone under questioning</h2></div>{usableCalls.length > 1 ? <select value={callIndex} onChange={(event) => { setCallIndex(Number(event.target.value)); setTurnLimit(20); }} className="rounded-xl border border-white/10 bg-neutral-900 px-3 py-2 text-xs text-neutral-300 outline-none">{usableCalls.map((item, index) => <option key={`${item.source_url}-${index}`} value={index}>{item.quarter || `Call ${index + 1}`} · {dateOnly(item.date)}</option>)}</select> : null}</div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="kpi-label">Uncertainty</div><div className="mt-1 font-mono text-lg text-neutral-200">{pct(finite(summary.uncertainty_turn_rate))}</div></div><div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="kpi-label">Forward-looking</div><div className="mt-1 font-mono text-lg text-neutral-200">{pct(finite(summary.forward_looking_turn_rate))}</div></div><div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="kpi-label">Transcript words</div><div className="mt-1 font-mono text-lg text-neutral-200">{call.transcript_word_count == null ? "—" : numberFormat.format(call.transcript_word_count)}</div></div><div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="kpi-label">Source</div>{call.source_url ? <a href={call.source_url} target="_blank" rel="noreferrer" className="mt-1 block truncate text-sm text-sky-300 hover:underline">{call.source || "Public transcript"}</a> : <div className="mt-1 text-sm text-neutral-500">{call.source || "—"}</div>}</div></div>
      </div>
