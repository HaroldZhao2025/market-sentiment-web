...
function SentimentChart({ rows }: { rows: DailyRow[] }) {
  const data = rows.filter((row) => finite(row.sentiment_equal_weighted) != null).slice(-520);
  if (!data.length) return <div className="grid h-72 place-items-center text-sm text-neutral-600">No sentiment history.</div>;
...
export default function IndexSentimentClient({ indexes }: { indexes: IndexSeries[] }) {
  ...
  if (!current || !metrics) return <div className="card p-5 text-neutral-500">No index sentiment artifact.</div>;
  return <div className="space-y-6" data-no-translate="true">
    ...
    <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
      ...
      <div className="kpi"><div className="kpi-label">{t("Observed companies","已观测公司")}</div><div className="kpi-value">{metrics.latest?.observed_tickers?.toLocaleString()??"—"}</div><div className="kpi-sub">{t("of","共")} {current.member_count.toLocaleString()} {t("members","家公司")}</div></div>
    ...
    <section className="ambient-panel p-4 md:p-6"><div className="mb-4"><div className="eyebrow">{t("Historical sentiment","历史情绪")}</div><h2 className="section-title mt-1">{current.name}</h2><p className="section-copy">{t("Equal-weight sentiment across constituents with fresh observed company news on each date. Missing company-days are excluded, never filled with zero.","每天对有真实新闻情绪观测的成分公司做等权聚合。没有新闻的公司日会被排除，绝不会填成 0。")}</p></div><SentimentChart rows={current.daily}/></section>
