...
export default function HomeClient() {
  ...
  useEffect(() => {
    const url = `${BASE}/data/_tickers.json`;
    fetch(url, { cache: "no-store" })
      .then((r) => {
        if (!r.ok) throw new Error(`HTTP ${r.status} for ${url}`);
        return r.json();
      })
      .then((j) => (Array.isArray(j) ? setTickers(j) : setErr("Bad _tickers.json shape")))
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      ...
      {err && <div className="text-sm text-red-600">Couldn’t load tickers: {err}</div>}
      {!err && tickers === null && <div className="text-sm text-neutral-500">Loading…</div>}

      {Array.isArray(tickers) && tickers.length > 0 ? (
        <>
          <div className="text-xs text-neutral-500">
            Loaded {tickers.length} tickers from <code>{BASE}/data/_tickers.json</code>
          </div>
