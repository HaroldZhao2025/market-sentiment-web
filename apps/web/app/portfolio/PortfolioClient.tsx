...
function ma7(arr: number[]) {
  ...
  for (let i = 0; i < arr.length; i++) { const v = Number(arr[i] || 0); run += v; if (i >= 7) run -= Number(arr[i - 7] || 0); out.push(i >= 6 ? run / 7 : NaN); }
  return out;
}
const label = (v: number) => (v >= 0.4 ? "Strong Positive" : v >= 0.1 ? "Positive" : v <= -0.4 ? "Strong Negative" : v <= -0.1 ? "Negative" : "Neutral");
const recommendation = (v: number) => (v >= 0.4 ? "Strong Buy" : v >= 0.1 ? "Buy" : v <= -0.4 ? "Strong Sell" : v <= -0.1 ? "Sell" : "Hold");
...
export default function TickerClient({ symbol, series, news }: { symbol: string; series: SeriesIn; news: NewsItem[]; }) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 space-y-8">
      ...
      <div className="rounded-2xl p-6 shadow-sm border bg-white overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="rounded-2xl p-5 shadow-sm border bg-white">
          ...
          <div className="text-2xl md:text-3xl font-semibold">{label(lastS)} <span className="text-neutral-500 text-lg">({lastS.toFixed(2)})</span></div>
        </div>
        <div className="rounded-2xl p-5 shadow-sm border bg-white">
      ...
      <div className="rounded-2xl p-6 shadow-sm border bg-white">
        <div className="flex items-baseline justify-between">
          ...
          <p className="text-xs text-neutral-500">The list shows the most recent headlines; the chart sentiment uses headlines across the full period.</p>
        </div>
        {news?.length ? (
