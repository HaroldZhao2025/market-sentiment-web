...
export default function CompaniesPage() {
  ...
  const coverageMap = new Map((Array.isArray(earningsCoverage.companies) ? earningsCoverage.companies : []).filter((row) => row?.ticker).map((row) => [String(row.ticker).toUpperCase(), row] as const));
  const rows: CompanyRowV2[] = baseRows.map((row) => {
    const call = coverageMap.get(String(row.ticker || "").toUpperCase());
    return { ...row, call_status: call?.status ?? "no_structured_call", complete_calls: Number(call?.complete_calls || 0), structured_calls: Number(call?.structured_calls || 0), call_links: Number(call?.call_links || 0), call_source: call?.source || "" };
  });
  ...
  const broader = Math.max(0, rows.length - sp500 - midcap - smallcap);
  ...
  return <main className="space-y-7">
    <section><div className="eyebrow">U.S. company universe</div><h1 className="page-title mt-2">Companies</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-neutral-400">Search the broad U.S. company layer, including the S&amp;P Composite 1500 and additional names. S&amp;P 500 index calculations remain separate.</p></section>

    <CompanyMetrics total={rows.length} sp500={sp500} midcap={midcap} smallcap={smallcap} broader={broader} />

    {rows.length ? <>
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <div className="kpi"><div className="kpi-label">Structured calls</div><div className="kpi-value text-white">{completeCalls.toLocaleString("en-US")}</div><div className="kpi-sub">{(callCoverage * 100).toFixed(1)}% company coverage</div></div>
        <div className="kpi"><div className="kpi-label">News ready</div><div className="kpi-value text-white">{newsReady.toLocaleString("en-US")}</div><div className="kpi-sub">{(newsRate * 100).toFixed(1)}% with retained news</div></div>
        <div className="kpi"><div className="kpi-label">Price history ready</div><div className="kpi-value text-white">{historyReady.toLocaleString("en-US")}</div><div className="kpi-sub">{(historyRate * 100).toFixed(1)}% with extended daily history</div></div>
        <div className="kpi"><div className="kpi-label">Source policy</div><div className="mt-2 text-xl font-semibold text-emerald-300">Free sources only</div><div className="kpi-sub">No paid/Premium news or transcript feed</div></div>
      </section>
