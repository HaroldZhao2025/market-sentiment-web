  ...
  return (
    <section className="space-y-3">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div><div className="eyebrow">News archive</div><h2 className="section-title mt-1">Retained headlines</h2><p className="section-copy">Free-public company news, newest first.</p></div>
        <input value={query} onChange={(event) => { setQuery(event.target.value); setLimit(25); }} placeholder="Search headlines" className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-xs text-white outline-none placeholder:text-neutral-600 md:w-64" />
      </div>

      <div className="table-shell overflow-x-auto">
        <table className="w-full min-w-[760px] text-sm">
          <thead className="border-b border-white/10 bg-white/[0.025] text-left text-[11px] uppercase tracking-[0.12em] text-neutral-600"><tr><th className="px-4 py-3">Date</th><th className="px-4 py-3">Headline</th><th className="px-4 py-3">Source</th><th className="px-4 py-3 text-right">Score</th></tr></thead>
          <tbody>{visible.map((item, index) => { const score = finite(item.s); return <tr key={`${item.url}-${index}`} className="border-b border-white/[0.06] last:border-0 hover:bg-white/[0.025]"><td className="whitespace-nowrap px-4 py-3 font-mono text-xs text-neutral-600">{dateOnly(item.ts)}</td><td className="max-w-2xl px-4 py-3"><a href={item.url} target="_blank" rel="noreferrer" className="font-medium leading-6 text-neutral-200 hover:text-white hover:underline">{item.title}</a></td><td className="px-4 py-3 text-xs text-neutral-500">{sourceOf(item)}</td><td className={`px-4 py-3 text-right font-mono text-xs ${scoreClass(score)}`}>{score == null ? "—" : `${score > 0 ? "+" : ""}${score.toFixed(4)}`}</td></tr>; })}</tbody>
        </table>
      </div>
