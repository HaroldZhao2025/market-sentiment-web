...
function lastFinite(values: number[], offset = 0): number | null {
  let seen = 0;
...
function DriverCard({ item, positive }: { item: NewsItem; positive: boolean }) {
  const score = finite(item.s);
  return <a href={item.url} target="_blank" rel="noreferrer" className="card card-hover block p-4">
    <div className="flex items-start justify-between gap-3"><span className={`rounded-full px-2 py-1 text-[10px] font-semibold uppercase tracking-wider ${positive ? "bg-emerald-400/10 text-emerald-300" : "bg-rose-400/10 text-rose-300"}`}>{positive ? "Positive driver" : "Negative driver"}</span><span className={`font-mono text-xs ${tone(score)}`}>{signed(score, 3)}</span></div>
...
export default function TickerClientV2({ series, news, newsTotal = 0 }: { symbol: string; series: SeriesIn; news: NewsItem[]; newsTotal?: number }) {
  ...
  const aligned = useMemo(() => ({ date: series.date.slice(0, n), price: series.price.slice(0, n), sentiment: series.sentiment.slice(0, n) }), [series, n]);
  const sentimentMA7 = useMemo(() => rollingObservedMean(aligned.sentiment, 7), [aligned.sentiment]);
  const sentimentObserved = useMemo(() => observedValues(aligned.sentiment), [aligned.sentiment]);
  const latestSent = lastFinite(aligned.sentiment);
  const priorSent = lastFinite(aligned.sentiment, 1);
  const sentimentChange = latestSent != null && priorSent != null ? latestSent - priorSent : null;
  const latestMA7 = lastFinite(sentimentMA7);
  const latestPrice = lastFinite(aligned.price);
  const priorPrice = lastFinite(aligned.price, 1);
  ...
  return <div className="space-y-8">
    <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      ...
      <div className="kpi"><div className="kpi-label">7-observation average</div><div className={`kpi-value ${tone(latestMA7)}`}>{signed(latestMA7)}</div><div className="kpi-sub">No-news days are skipped</div></div>
    ...
    <section className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="card p-5"><div className="kpi-label">Signal observations</div><div className="text-2xl font-semibold text-white">{sentimentObserved.length.toLocaleString()}</div><div className="mt-1 text-xs text-neutral-600">Across {aligned.date.length.toLocaleString()} trading days</div></div>
      </div>
      {themes.length ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{themes.slice(0, 6).map((theme) => <div key={theme.name} className="card p-4"><div className="flex items-start justify-between gap-3"><div><div className="text-sm font-semibold text-neutral-200">{theme.name}</div><div className="mt-1 text-[11px] text-neutral-600">{theme.count} scored item{theme.count === 1 ? "" : "s"}</div></div><div className={`font-mono text-xs ${tone(theme.score)}`}>{signed(theme.score, 3)}</div></div></div>)}</div> : null}
    ...
    <section className="space-y-3">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><div className="eyebrow">History</div><h2 className="section-title mt-1">Price and observed sentiment</h2></div><div className="flex flex-wrap items-center gap-3"><ChartLegend /><div className="flex rounded-xl border border-white/10 bg-black/30 p-1"><button type="button" onClick={() => setMode("overlay")} className={`rounded-lg px-3 py-2 text-xs ${mode === "overlay" ? "bg-white/10 text-white" : "text-neutral-500"}`}>Overlay</button><button type="button" onClick={() => setMode("separate")} className={`rounded-lg px-3 py-2 text-xs ${mode === "separate" ? "bg-white/10 text-white" : "text-neutral-500"}`}>Separate</button></div></div></div>
      <div className="ambient-panel p-3 md:p-5"><LineChart mode={mode} dates={aligned.date} price={aligned.price} sentiment={aligned.sentiment} sentimentMA7={sentimentMA7} height={540} /></div>
    </section>
  </div>;
}