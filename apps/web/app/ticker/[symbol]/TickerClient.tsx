  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Market Sentiment — {symbol}</h1>
          <p className="text-sm text-neutral-500 mt-1">Daily sentiment (FinBERT) aligned to price, last 12 months.</p>
        </div>
        <div className="inline-flex items-center rounded-xl bg-neutral-100 p-1">
          <button
            className={`px-3 py-1.5 text-sm rounded-lg transition ${mode === "separate" ? "bg-black text-white shadow-sm" : "text-neutral-700"}`}
            onClick={() => setMode("separate")}
          >
            Separate
          </button>
          <button
            className={`px-3 py-1.5 text-sm rounded-lg transition ${mode === "overlay" ? "bg-black text-white shadow-sm" : "text-neutral-700"}`}
      ...
      <div className="rounded-2xl p-6 shadow-sm border bg-white">
        <h3 className="font-semibold mb-3">Sentiment and Price</h3>
        <LineChart mode={mode} dates={series.date} price={series.price} sentiment={series.sentiment} sentimentMA7={sMA7} height={520} />
      </div>
      ...
      <div className="rounded-2xl p-6 shadow-sm border bg-white">
        <div className="flex items-baseline justify-between">
          <h3 className="font-semibold mb-1">Recent Headlines</h3>
          <p className="text-xs text-neutral-500">The chart uses all available headlines; list below shows the latest ones.</p>
        </div>
        {news?.length ? (
