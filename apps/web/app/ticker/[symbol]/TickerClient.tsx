  ...
  return (
    <div style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 12 }}>
      ...
      <svg viewBox={`0 0 ${width} ${height}`} style={{ width: "100%", display: "block" }}
           onMouseLeave={() => setHoverI(null)} role="img" aria-label="Sentiment and price">
        {showSent && <line x1={pad.l} x2={width - pad.r} y1={sY(0)} y2={sY(0)} stroke="#e5e7eb" />}
        {showSent && series.sentiment.map((v, i) => {
          const x = xAt(i);
        ...
        {showPrice && series.price.map((v, i) => {
          if (i === 0) return null;
          ...
          <>
            ...
            {showPrice && (
              <>
                ...
                <b>Pred. return:</b> {toPct(((series.price[hoverI + 1] ?? series.price[hoverI]) - series.price[hoverI]) / Math.max(1e-6, series.price[hoverI]))}
              </>
            )}
          </>
        ) : (<>Hover to see values.</>)}
    ...
    <div style={{ display: "grid", gap: 16 }}>
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        <div role="tablist" aria-label="View mode"
             style={{ border: "1px solid #e5e7eb", borderRadius: 999, overflow: "hidden" }}>
