    ...
    <div className="mx-auto max-w-6xl px-4 py-8 space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="inline-flex items-center rounded-xl bg-neutral-100 p-1">
          <button className={`px-3 py-1.5 text-sm rounded-lg transition ${mode === "separate" ? "bg-black text-white shadow-sm" : "text-neutral-700"}`} onClick={() => setMode("separate")}>Separate View</button>
          <button className={`px-3 py-1.5 text-sm rounded-lg transition ${mode === "overlay" ? "bg-black text-white shadow-sm" : "text-neutral-700"}`} onClick={() => setMode("overlay")}>Overlayed View</button>
        </div>
      </div>
      ...
      <div className="rounded-2xl p-6 shadow-sm border bg-white">
        ...
        <LineChart mode={mode} dates={series.date} price={series.price} sentiment={series.sentiment} sentimentMA7={sMA7} height={460} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="rounded-2xl p-5 shadow-sm border bg-white">
          ...
          <div className="text-2xl md:text-3xl font-semibold">
            {band(lastS)} <span className="text-neutral-500 text-lg">({lastS.toFixed(4)})</span>
          </div>
        </div>
        <div className="rounded-2xl p-5 shadow-sm border bg-white">
        ...
        <div className="rounded-2xl p-5 shadow-sm border bg-white">
          ...
          <div className="text-2xl md:text-3xl font-semibold">{advice(lastMA)}</div>
        </div>
        <div className="rounded-2xl p-5 shadow-sm border bg-white">
      ...
      <div className="rounded-2xl p-6 shadow-sm border bg-white">
        <div className="flex items-baseline justify-between">
          ...
          <p className="text-xs text-neutral-500">The list shows the most recent headlines; the chart uses news across the full period.</p>
        </div>
        {news?.length ? (
          <ul className="mt-2 space-y-2">
            {news.slice(0, 12).map((n, i) => (
              <li key={i} className="text-sm leading-6">
                ...
                <a className="underline decoration-dotted underline-offset-2" href={n.url} target="_blank" rel="noreferrer">{n.title}</a>
              </li>
            ))}
          </ul>
        ) : (
