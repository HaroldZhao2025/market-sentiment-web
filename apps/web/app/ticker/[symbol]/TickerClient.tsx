    ...
    <div className="mx-auto max-w-6xl px-4 py-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="rounded-2xl p-4 shadow-sm border bg-white">
          ...
          <div className="text-2xl font-semibold mt-1">{(lastMA * 100).toFixed(2)}%</div>
        </div>
        <div className="rounded-2xl p-4 shadow-sm border bg-white">
          <div className="text-sm text-neutral-500">Our Recommendation</div>
          <div className="text-2xl font-semibold mt-1">{lastMA >= 0 ? "Buy" : "Hold"}</div>
        </div>
      </div>
      ...
      <div className="rounded-2xl p-4 shadow-sm border bg-white">
        ...
        <div className="space-y-2">
          {(news || []).slice(-50).reverse().map((n, i) => (
            <div key={i} className="text-sm">
              <span className="text-neutral-500 mr-2">{n.ts}</span>
