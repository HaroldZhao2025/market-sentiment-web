...
// ---------- utility: robust base prefix on GitHub Pages ----------
// On GH Pages, the site is hosted at /<repo-name>/... so absolute "/data/..."
// would break. We derive the prefix from the current path (before "/ticker/").
function basePrefixFromLocation(): string {
  if (typeof window === "undefined") return "";
  const path = window.location.pathname;
  // e.g., "/market-sentiment-web/ticker/AAPL" -> "/market-sentiment-web"
...
function buildSeries(obj: any): SeriesIn | null {
  ...
  const price = numArr(obj.price ?? obj.prices ?? obj.close ?? []);
  const sentiment = numArr(obj.sentiment ?? obj.sentiments ?? obj.s ?? []);
...
function SentimentPriceChart({
  ...
  const sAbsMax = Math.max(1, Math.max(...series.sentiment.map((x) => Math.abs(x))));
  const sY = (v: number) => {
      ...
      {mode === "overlay" ? (
        ...
        >
          ...
          {series.sentiment.map((v, i) => {
            ...
            return (
              <rect key={`s${i}`} x={x - 1} y={y} width={2} height={Math.max(1, h)} fill="#6b47dc" opacity={0.6} />
            );
          })}
          {series.price.map((v, i) => {
            if (i === 0) return null;
            const x1 = xs(i - 1), y1 = pY(series.price[i - 1]);
            const x2 = xs(i), y2 = pY(v);
            return <line key={`p${i}`} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#10b981" strokeWidth={1.5} />;
          })}
          {hoverI != null && (
            <line
              x1={xs(hoverI)} x2={xs(hoverI)} y1={pad.t} y2={height - pad.b}
          ...
          {series.date.map((_, i) => {
            ...
            return (
              <rect
                key={`h${i}`} x={x - w / 2} y={pad.t} width={w} height={height - pad.t - pad.b}
        ...
        <div style={{ display: "grid", gridTemplateRows: "1fr 1fr", gap: 8 }}>
          <svg viewBox={`0 0 ${width} 160`} style={{ width: "100%", display: "block" }}
               onMouseLeave={() => setHoverI(null)}>
            <line x1={pad.l} x2={width - pad.r} y1={80} y2={80} stroke="#e5e7eb" />
            {series.sentiment.map((v, i) => {
              const x = pad.l + (i * (width - pad.l - pad.r)) / Math.max(1, n - 1);
              const y0 = 80, y1 = 80 - (v / sAbsMax) * 70;
              const y = Math.min(y0, y1), h = Math.abs(y1 - y0);
              return <rect key={i} x={x - 1} y={y} width={2} height={Math.max(1, h)} fill="#6b47dc" opacity={0.6} />;
            })}
          </svg>
          <svg viewBox={`0 0 ${width} 160`} style={{ width: "100%", display: "block" }}
               onMouseLeave={() => setHoverI(null)}>
            {series.price.map((v, i) => {
              if (i === 0) return null;
              const x1 = pad.l + ((i - 1) * (width - pad.l - pad.r)) / Math.max(1, n - 1);
              const x2 = pad.l + (i * (width - pad.l - pad.r)) / Math.max(1, n - 1);
              const pMin = Math.min(...series.price), pMax = Math.max(...series.price);
              const y = (vv: number) =>
                pad.t + ((pMax - vv) * (160 - pad.t - 20)) / Math.max(1e-6, pMax - pMin);
              return <line key={i} x1={x1} y1={y(series.price[i - 1])} x2={x2} y2={y(v)} stroke="#10b981" strokeWidth={1.5} />;
            })}
          </svg>
        </div>
      )}
  ...
  return (
    <span
      style={{
        ...
        fontSize: 12, fontWeight: 600, color: "#ffffff", background: color, lineHeight: "18px",
      }}
    >
      {children}
    </span>
...
function InsightCards({ series }: { series: SeriesIn }) {
  ...
  const ret = last > 0 ? (series.price[last] - series.price[last - 1]) / Math.max(1e-6, series.price[last - 1]) : 0;
  ...
  const advisory =
    sAvg > 0.4 ? "Strong Buy" : sAvg > 0.1 ? "Buy" : sAvg < -0.4 ? "Strong Sell" : sAvg < -0.1 ? "Sell" : "Hold";
          ...
          <tbody>
            {items.map((n, i) => (
              <tr key={i} style={{ borderBottom: "1px solid #f3f4f6" }}>
                <td style={{ padding: "8px", fontSize: 13, color: "#374151" }}>{n.ts}</td>
...
export default function TickerClient({ symbol }: Props) {
  const [series, setSeries] = React.useState<SeriesIn | null>(null);
  const [news, setNews] = React.useState<NewsItem[]>([]);
  const [mode, setMode] = React.useState<"overlay" | "separate">("overlay");
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      // primary path: prefix/data/...
      const urlTicker = `${prefix}/data/ticker/${symbol}.json`;
      const urlNews = `${prefix}/data/news/${symbol}.json`;

      // fallback path for local dev (where prefix === "")
      const fbTicker = `/data/ticker/${symbol}.json`;
      const fbNews = `/data/news/${symbol}.json`;

      const j =
        (await fetchJSON<any>(urlTicker)) ||
        (await fetchJSON<any>(fbTicker));
      const s = buildSeries(j);

      if (!cancelled) {
        if (s) {
          setSeries(s);
          const nn =
            (await fetchJSON<NewsItem[]>(urlNews)) ||
            (await fetchJSON<NewsItem[]>(fbNews)) ||
            [];
          setNews(Array.isArray(nn) ? nn : []);
          setErr(null);
        } else {
          setErr(
            `Could not parse data for ${symbol}. Expected arrays: date[], price[], sentiment[].`
          );
        }
      }
    })();
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        >
          {(["overlay", "separate"] as const).map((m) => (
            <button
              key={m}
              onClick={() => setMode(m)}
              style={{
