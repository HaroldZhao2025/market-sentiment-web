...
type TickerJSON = {
  dates?: string[];  // or "date"
  date?: string[];
  price?: number[];  // or "close"
  close?: number[];
  S?: number[];            // or "sentiment"
  sentiment?: number[];
  S_ma7?: number[];        // or "sentiment_ma7"
...
export default function TickerClient({ symbol }: { symbol: string }) {
  ...
  useEffect(() => {
    ...
    const url = `data/ticker/${symbol}.json`;
  ...
  const series = useMemo(() => {
    const dates = (data?.dates ?? data?.date ?? []) as string[];
    const price = (data?.price ?? data?.close ?? []) as number[];
    const sRaw  = (data?.S ?? data?.sentiment ?? []) as number[];
    const sMa7  = (data?.S_ma7 ?? data?.sentiment_ma7 ?? []) as number[];

    const n = Math.min(dates.length || 0, price.length || 0, sRaw.length || 0);
    return {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      ...
      {!err && !hasData && (
        <div className="text-sm text-neutral-500">No series found for this ticker.</div>
      )}

      {hasData && chartData && (
        <>
          <div className="text-xs text-neutral-500">
            Loaded {series.dates.length} points from <code>data/ticker/{symbol}.json</code>
          </div>
