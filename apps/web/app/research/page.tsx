...
export default async function ResearchPage() {
  const [items, sections] = await Promise.all([loadResearchIndex(), loadResearchOverview()]);
...
python src/market_sentiment/cli/build_research.py --data-root data --out-dir apps/web/public/research
        ...
        <div className="space-y-10">
          {sections.map((sec) => {
            const secItems = sec.slugs.map((s) => bySlug.get(s)).filter(Boolean);
                    ...
                    >
                      ...
                      {it.key_stats?.length ? (
                        <div className="mt-4 grid grid-cols-2 gap-2">
                          {it.key_stats.slice(0, 4).map((s: any) => (
