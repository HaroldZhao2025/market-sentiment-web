...
function dailySpreads(obs: PanelObs[], quantile: LabV2Quantile): DailySpread[] {
  ...
  for (const [date, rows] of Array.from(byDate.entries()).sort((a, b) => a[0].localeCompare(b[0]))) {
    ...
    const lowRows = sorted.slice(0, k);
    const highRows = sorted.slice(-k);
...
function sideTurnover(previous: string[], current: string[]): number {
  if (!previous.length || !current.length) return 1;
  const names = new Set([...previous, ...current]);
  let absoluteChange = 0;
  for (const symbol of names) {
    const oldWeight = previous.includes(symbol) ? 1 / previous.length : 0;
    const newWeight = current.includes(symbol) ? 1 / current.length : 0;
    absoluteChange += Math.abs(newWeight - oldWeight);
  }
...
function neweyWestMeanSe(values: number[], lag: number): number | null {
  ...
  const residuals = values.map((x) => x - mu);
  let longRunVariance = residuals.reduce((sum, e) => sum + e * e, 0) / n;
  const maxLag = Math.min(Math.max(0, lag), n - 1);
  for (let ell = 1; ell <= maxLag; ell += 1) {
    let gamma = 0;
    for (let t = ell; t < n; t += 1) gamma += residuals[t] * residuals[t - ell];
    gamma /= n;
    const bartlett = 1 - ell / (maxLag + 1);
    longRunVariance += 2 * bartlett * gamma;
  }
...
function summarize(panel: PanelObs[], dailyAll: DailySpread[], signal: LabV2Signal, horizon: LabV2Horizon, sector: string, quantile: LabV2Quantile, sample: LabV2Sample): LabV2Summary {
  ...
  const sd = sampleStd(spreads);
  const simpleT = spread != null && sd != null && sd > 0 ? spread / (sd / Math.sqrt(spreads.length)) : null;
  const hacLag = Math.min(Math.max(0, horizon - 1), Math.max(0, spreads.length - 1));
  const hacSe = neweyWestMeanSe(spreads, hacLag);
  const hacT = spread != null && hacSe != null && hacSe > 0 ? spread / hacSe : null;
  ...
  return {
    ...
    spread,
    simple_t_stat: simpleT,
    hac_t_stat: hacT,
    hac_se: hacSe,
    hac_lag: hacLag,
    hit_rate: spreads.length ? spreads.filter((x) => x > 0).length / spreads.length : null,
    sharpe,
    avg_turnover: averageTurnover(daily),
    n: scopedPanel.length,
...
export function buildLabV2Summaries(): LabV2Summary[] {
  const signals: LabV2Signal[] = ["sentiment", "sentiment_change", "divergence"];
  const horizons: LabV2Horizon[] = [1, 3, 5, 20];
  const quantiles: LabV2Quantile[] = [0.2, 0.25, 0.33];
  ...
  for (const signal of signals) {
    for (const horizon of horizons) {
      ...
      for (const sector of sectors) {
        const panel = sector === "All" ? fullPanel : fullPanel.filter((x) => x.sector === sector);
        for (const quantile of quantiles) {
          const daily = dailySpreads(panel, quantile);
          for (const sample of samples) output.push(summarize(panel, daily, signal, horizon, sector, quantile, sample));
        }
      }
    }
  }
